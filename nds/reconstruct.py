...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_scale', type=int, default=0.25, help="Scale applied to the input images. The factor is 1/image_scale, so image_scale=2 halves the image size")
    ...
    parser.add_argument('--noise_type', type=str, default=None, help="Const or random noise")
    parser.add_argument('--noise_size', type=float, default=0.1, help="Size of noise relative to base size")
    parser.add_argument('--num_views', type=int, default=None, help="Number of views to use for the reconstruction")
    ...
    # Read the views
    views = read_views(args.input_dir, scale=args.image_scale, device=device, noise_type=args.noise_type, noise_size=args.noise_size, num_views=args.num_views)
