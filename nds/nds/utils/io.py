...
def read_views(directory, scale, device, noise_type=None, noise_size=0, num_views=None):
    directory = Path(directory)
    if num_views:
        image_paths = [path for path in directory.iterdir() if (path.is_file() and path.suffix == '.png')]
    ...
    for image_path in image_paths:
        views.append(View.load(image_path, device, noise_type, noise_size))
